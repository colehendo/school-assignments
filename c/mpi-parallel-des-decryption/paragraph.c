...
int main(int argc, char **argv) {
  ...
  char *buf, *cbuf, *ubuf;  // buffers for encrypt/decrypt
  char *cpass;              // encrypted password
  char salt[3] = "H2";      // crypt salt
  struct stat st;
 ...
 */
  ...
  strncpy(salt, argv[2], sizeof(salt));
 ...
 */
  ...
  clen = len + extlen; // account for \0 terminator
 ...
 buf  = malloc(st.st_size+1);
  ...
  // encrypt file data
  cbuf = decrypt_des(pass, buf, st.st_size+1);
